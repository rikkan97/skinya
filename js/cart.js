...
function renderCheckout(){
  ...
  if(itemsEl){
    itemsEl.innerHTML = cart.length ? cart.map(i=>{
      // Καθαρό display: αν το προϊόν έχει bundle/sale έκπτωση, η blended
      // unit τιμή (π.χ. 9.03) μπερδεύει. Δείξε «qty τεμ. · σετ» αντί «qty × 9.03».
      ...
      const sub = hasDisc
        ? `${i.qty} τεμ. · <span style="color:var(--gold-leaf,#c0857a)">σετ ✦</span>`
    ...
    }).join('') : '<div class="cart-empty">Το καλάθι σου είναι άδειο.</div>';
  }
